...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-[var(--scrim)] transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Popup
        ...
        data-side={side}
        className={cn(
          // NOTE: width is intentionally NOT set here — every caller picks its
          // own via className (e.g. MobileNav uses w-72 sm:max-w-sm, the
          // trade detail sheet uses a viewport-percentage clamp). Keeping this
          // primitive width-agnostic means no fights with hard-coded
          // sm:max-w-sm defaults when a richer side panel is needed.
